import { MTaddChatRoomAdmin, MTaddMembersToChatRoomAllowList, MTblockChatRoomMembers, MTchangeChatRoomDescription, MTchangeChatRoomOwner, MTchangeChatRoomSubject, MTchatRoomChange, MTcreateChatRoom, MTdestroyChatRoom, MTfetchChatRoomAllowListFromServer, MTfetchChatRoomAnnouncement, MTfetchChatRoomAttributes, MTfetchChatRoomBlockList, MTfetchChatRoomInfoFromServer, MTfetchChatRoomMembers, MTfetchChatRoomMuteList, MTfetchPublicChatRoomsFromServer, MTgetChatRoom, MTisMemberInChatRoomAllowListFromServer, MTjoinChatRoom, MTleaveChatRoom, MTmuteAllChatRoomMembers, MTmuteChatRoomMembers, MTremoveChatRoomAdmin, MTremoveChatRoomAttributes, MTremoveChatRoomMembers, MTremoveMembersFromChatRoomAllowList, MTsetChatRoomAttributes, MTunBlockChatRoomMembers, MTunMuteAllChatRoomMembers, MTunMuteChatRoomMembers, MTupdateChatRoomAnnouncement } from './__internal__/Consts';
...
export class ChatRoomManager extends Native {
  ...
  setNativeListener(event) {
    ...
    this._roomSubscriptions.set(MTchatRoomChange, event.addListener(MTchatRoomChange, params => {
    ...
    this._roomListeners.forEach(listener => {
      ...
      switch (contactEventType) {
        ...
        case 'onSpecificationChanged':
          (_listener$onSpecifica = listener.onSpecificationChanged) === null || _listener$onSpecifica === void 0 ? void 0 : _listener$onSpecifica.call(listener, new ChatRoom(params.room));
          break;
  ...
  async joinChatRoom(roomId) {
    ...
    let r = await Native._callMethod(MTjoinChatRoom, {
      [MTjoinChatRoom]: {
        roomId: roomId
      }
    });
  ...
  async leaveChatRoom(roomId) {
    ...
    let r = await Native._callMethod(MTleaveChatRoom, {
      [MTleaveChatRoom]: {
        roomId
      }
    });
  ...
  async fetchPublicChatRoomsFromServer() {
    ...
    chatlog.log(`${ChatRoomManager.TAG}: fetchPublicChatRoomsFromServer: `, pageNum, pageSize);
    let r = await Native._callMethod(MTfetchPublicChatRoomsFromServer, {
    ...
    ChatRoomManager.checkErrorFromResult(r);
    let ret = new ChatPageResult({
      pageCount: r === null || r === void 0 ? void 0 : r[MTfetchPublicChatRoomsFromServer].count,
      list: r === null || r === void 0 ? void 0 : r[MTfetchPublicChatRoomsFromServer].list,
      opt: {
        map: param => {
  ...
  async fetchChatRoomInfoFromServer(roomId) {
    chatlog.log(`${ChatRoomManager.TAG}: fetchChatRoomInfoFromServer: ${roomId}`);
    let r = await Native._callMethod(MTfetchChatRoomInfoFromServer, {
      [MTfetchChatRoomInfoFromServer]: {
        roomId
      }
    });
    ChatRoomManager.checkErrorFromResult(r);
    const rr = r === null || r === void 0 ? void 0 : r[MTfetchChatRoomInfoFromServer];
    if (rr) {
  ...
  async getChatRoomWithId(roomId) {
    ...
    let r = await Native._callMethod(MTgetChatRoom, {
      [MTgetChatRoom]: {
        roomId
      }
    });
    ChatRoomManager.checkErrorFromResult(r);
    const rr = r === null || r === void 0 ? void 0 : r[MTgetChatRoom];
    if (rr) {
      return new ChatRoom(rr);
    }
    return undefined;
  ...
  async createChatRoom(subject, description, welcome, members) {
    ...
    chatlog.log(`${ChatRoomManager.TAG}: createChatRoom: `, subject, description, welcome, members, maxCount);
    let r = await Native._callMethod(MTcreateChatRoom, {
      [MTcreateChatRoom]: {
        subject: subject,
        desc: description,
        welcomeMsg: welcome,
    ...
    ChatRoomManager.checkErrorFromResult(r);
    let ret = new ChatRoom(r === null || r === void 0 ? void 0 : r[MTcreateChatRoom]);
    return ret;
  }
  ...
  async destroyChatRoom(roomId) {
    ...
    let r = await Native._callMethod(MTdestroyChatRoom, {
      [MTdestroyChatRoom]: {
        roomId
      }
    });
  ...
  async changeChatRoomSubject(roomId, subject) {
    chatlog.log(`${ChatRoomManager.TAG}: changeChatRoomSubject: ${roomId}, ${subject}`);
    let r = await Native._callMethod(MTchangeChatRoomSubject, {
  ...
  async changeChatRoomDescription(roomId, description) {
    chatlog.log(`${ChatRoomManager.TAG}: changeChatRoomSubject: `, roomId, description);
    let r = await Native._callMethod(MTchangeChatRoomDescription, {
  ...
  async fetchChatRoomMembers(roomId) {
    ...
    chatlog.log(`${ChatRoomManager.TAG}: fetchChatRoomMembers: `, roomId, cursor, pageSize);
    let r = await Native._callMethod(MTfetchChatRoomMembers, {
    ...
    let ret = new ChatCursorResult({
      cursor: r === null || r === void 0 ? void 0 : r[MTfetchChatRoomMembers].cursor,
      list: r === null || r === void 0 ? void 0 : r[MTfetchChatRoomMembers].list,
      opt: {
        map: param => {
          return param;
        }
  ...
  async muteChatRoomMembers(roomId, muteMembers) {
    ...
    chatlog.log(`${ChatRoomManager.TAG}: muteChatRoomMembers: `, roomId, muteMembers, duration);
    let r = await Native._callMethod(MTmuteChatRoomMembers, {
  ...
  async unMuteChatRoomMembers(roomId, unMuteMembers) {
    chatlog.log(`${ChatRoomManager.TAG}: unMuteChatRoomMembers: `, roomId, unMuteMembers);
    let r = await Native._callMethod(MTunMuteChatRoomMembers, {
  ...
  async changeOwner(roomId, newOwner) {
    ...
    let r = await Native._callMethod(MTchangeChatRoomOwner, {
  ...
  async addChatRoomAdmin(roomId, admin) {
    ...
    let r = await Native._callMethod(MTaddChatRoomAdmin, {
      [MTaddChatRoomAdmin]: {
        roomId,
        admin
      }
  ...
  async removeChatRoomAdmin(roomId, admin) {
    ...
    let r = await Native._callMethod(MTremoveChatRoomAdmin, {
  ...
  async fetchChatRoomMuteList(roomId) {
    ...
    let pageSize = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 200;
    chatlog.log(`${ChatRoomManager.TAG}: fetchChatRoomMuteList: `, roomId, pageNum, pageSize);
    let r = await Native._callMethod(MTfetchChatRoomMuteList, {
    ...
    ChatRoomManager.checkErrorFromResult(r);
    let ret = r === null || r === void 0 ? void 0 : r[MTfetchChatRoomMuteList];
    return ret;
  }
  ...
  async removeChatRoomMembers(roomId, members) {
    chatlog.log(`${ChatRoomManager.TAG}: removeChatRoomMembers: `, roomId, members);
    let r = await Native._callMethod(MTremoveChatRoomMembers, {
  ...
  async blockChatRoomMembers(roomId, members) {
    chatlog.log(`${ChatRoomManager.TAG}: blockChatRoomMembers: `, roomId, members);
    let r = await Native._callMethod(MTblockChatRoomMembers, {
  ...
  async unBlockChatRoomMembers(roomId, members) {
    chatlog.log(`${ChatRoomManager.TAG}: unBlockChatRoomMembers: `, roomId, members);
    let r = await Native._callMethod(MTunBlockChatRoomMembers, {
  ...
  async fetchChatRoomBlockList(roomId) {
    let pageNum = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 1;
    let pageSize = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 200;
    chatlog.log(`${ChatRoomManager.TAG}: fetchChatRoomBlockList: `, roomId, pageNum, pageSize);
    let r = await Native._callMethod(MTfetchChatRoomBlockList, {
    ...
    ChatRoomManager.checkErrorFromResult(r);
    const ret = r === null || r === void 0 ? void 0 : r[MTfetchChatRoomBlockList];
    return ret;
  }
  ...
  async updateChatRoomAnnouncement(roomId, announcement) {
    chatlog.log(`${ChatRoomManager.TAG}: updateChatRoomAnnouncement: `, roomId, announcement);
    let r = await Native._callMethod(MTupdateChatRoomAnnouncement, {
  ...
  async fetchChatRoomAnnouncement(roomId) {
    ...
    let r = await Native._callMethod(MTfetchChatRoomAnnouncement, {
      [MTfetchChatRoomAnnouncement]: {
        roomId
      }
    });
    ChatRoomManager.checkErrorFromResult(r);
    return r === null || r === void 0 ? void 0 : r[MTfetchChatRoomAnnouncement];
  }
  ...
  async fetchChatRoomAllowListFromServer(roomId) {
    chatlog.log(`${ChatRoomManager.TAG}: fetchChatRoomAllowListFromServer: `, roomId);
    let r = await Native._callMethod(MTfetchChatRoomAllowListFromServer, {
      [MTfetchChatRoomAllowListFromServer]: {
        roomId
      }
    });
    ChatRoomManager.checkErrorFromResult(r);
    let ret = r === null || r === void 0 ? void 0 : r[MTfetchChatRoomAllowListFromServer];
    return ret;
  }
  ...
  async isMemberInChatRoomAllowList(roomId) {
    chatlog.log(`${ChatRoomManager.TAG}: isMemberInChatRoomAllowList: `, roomId);
    let r = await Native._callMethod(MTisMemberInChatRoomAllowListFromServer, {
      [MTisMemberInChatRoomAllowListFromServer]: {
        roomId
      }
    });
    ChatRoomManager.checkErrorFromResult(r);
    let ret = r === null || r === void 0 ? void 0 : r[MTisMemberInChatRoomAllowListFromServer];
    return ret;
  }
  ...
  async addMembersToChatRoomAllowList(roomId, members) {
    chatlog.log(`${ChatRoomManager.TAG}: addMembersToChatRoomAllowList: `, roomId, members);
    let r = await Native._callMethod(MTaddMembersToChatRoomAllowList, {
  ...
  async removeMembersFromChatRoomAllowList(roomId, members) {
    chatlog.log(`${ChatRoomManager.TAG}: removeMembersFromChatRoomAllowList: `, roomId, members);
    let r = await Native._callMethod(MTremoveMembersFromChatRoomAllowList, {
  ...
  async muteAllChatRoomMembers(roomId) {
    ...
    let r = await Native._callMethod(MTmuteAllChatRoomMembers, {
  ...
  async unMuteAllChatRoomMembers(roomId) {
    ...
    let r = await Native._callMethod(MTunMuteAllChatRoomMembers, {
  ...
  async fetchChatRoomAttributes(roomId, keys) {
    ...
    let r = await Native._callMethod(MTfetchChatRoomAttributes, {
    ...
    ChatRoomManager.checkErrorFromResult(r);
    const ret = new Map();
    Object.entries(r === null || r === void 0 ? void 0 : r[MTfetchChatRoomAttributes]).forEach(v => {
      ret.set(v[0], v[1]);
    });
    return ret;
  }
  ...
  async addAttributes(params) {
    ...
    let r = await Native._callMethod(MTsetChatRoomAttributes, {
    ...
    ChatRoomManager.checkErrorFromResult(r);
    const ret = new Map();
    if (r !== null && r !== void 0 && r[MTsetChatRoomAttributes]) {
      Object.entries(r === null || r === void 0 ? void 0 : r[MTsetChatRoomAttributes]).forEach(v => {
        ret.set(v[0], v[1]);
      });
    }
    return ret;
  ...
  async removeAttributes(params) {
    ...
    let r = await Native._callMethod(MTremoveChatRoomAttributes, {
    ...
    ChatRoomManager.checkErrorFromResult(r);
    const ret = new Map();
    if (r !== null && r !== void 0 && r[MTremoveChatRoomAttributes]) {
      Object.entries(r === null || r === void 0 ? void 0 : r[MTremoveChatRoomAttributes]).forEach(v => {
        ret.set(v[0], v[1]);
      });
    }
    return ret;
