import { MTfetchUserInfoById, MTupdateOwnUserInfo } from './__internal__/Consts';
...
export class ChatUserInfoManager extends Native {
  ...
  async updateOwnUserInfo(params) {
    ...
    if (ret.has(userId)) {
      ...
      let r = await Native._callMethod(MTupdateOwnUserInfo, {
        [MTupdateOwnUserInfo]: {
          userInfo
        }
      });
  ...
  async fetchUserInfoById(userIds) {
    ...
    let r = await Native._callMethod(MTfetchUserInfoById, {
      [MTfetchUserInfoById]: {
        userIds: userIds
      }
    });
    ChatUserInfoManager.checkErrorFromResult(r);
    const ret = new Map();
    Object.entries(r === null || r === void 0 ? void 0 : r[MTfetchUserInfoById]).forEach(value => {
